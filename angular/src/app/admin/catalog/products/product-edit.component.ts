...
export class ProductEditComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    const payload = {
      ...
      variants: (v.variants ?? []).map((vr: { id?: string | null; sku: string; price?: number | null; quantity: number; dynamicAttributes?: Record<string, unknown>; attributes: { productAttributeId: string; value: string }[] }) => ({
        id: vr.id ?? null,
        sku: vr.sku ?? '',
        price: vr.price ?? null,
        ...
        attributes: (vr.attributes ?? []).map((a: { productAttributeId: string; value: string }) => ({
